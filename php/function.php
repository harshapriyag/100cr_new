<?php
    ...
    $insert_query = "INSERT INTO sp_index(
	...
	function getValues() {
		...
		$query = "select * from sp_index where id = $id";
    ...
    function editData() {
		...
		$update_query = "update sp_index set project_id = '$prj_id', 
		...
		if ($insert_index_data) {
			pg_query(DBCON,	"update sp_index set geom = ST_SetSRID(ST_MakePoint(longitude::float, latitude::float),4326) where id = $id");
		...
		if ($image1 != "undefined"){
			...
			pg_query(DBCON, "update sp_index set photo_prior_to_commencement_of_work = '$image1' where id = $id");
			
		}
		
		if ($image2 != "undefined"){
			
			pg_query(DBCON, "update sp_index set photo_current_status = '$image2' where id = $id");
  ...
  function getGeomCol($table) {
	...
	function getAllProjectsForReports(){
		...
		if($role == 11){
			$query = "select sp.department_name, sp.implementing_agency, sp.name_of_the_project, sp.funding_agency,sp.project_start_date,sp.project_end_date,sp.revised_start_date, sp.revised_end_date, sp.id, sp.project_cost, sp.present_status, cd.delay_desc,sp.other_reasons_delay,sp.cost_overrun,sp.cost_overrun_reasons,DATE(updated_time) updated_time  from sp_index as sp left join mst_causes_of_delay as cd on sp.causes_of_delay = cast(cd.delay_id as text)  order by sp.department_name,sp.id";
		}else{
			$hod_code = $_SESSION['100c_user_info']['hod_dept_code'];
			$query = "select sp.department_name, sp.implementing_agency, sp.name_of_the_project, sp.funding_agency,sp.project_start_date,sp.project_end_date,sp.revised_start_date, sp.revised_end_date, sp.id, sp.project_cost, sp.present_status, cd.delay_desc,sp.other_reasons_delay,sp.cost_overrun,sp.cost_overrun_reasons,DATE(updated_time) updated_time
			 from sp_index as sp left join mst_causes_of_delay as cd on sp.causes_of_delay = cast(cd.delay_id as text) where sp.hod_name='$hod_code' order by sp.department_name,sp.id";
		}
	...
	function getProjectNameByImplAgency(){
		if(isset($_GET['hId'])){
			$hod_code = $_GET['hId'];
			$query = "select project_id,name_of_the_project as project_name from sp_index where hod_name='$hod_code' ";
			$result = pg_query(DBCON, $query);
			$rs = pg_fetch_all($result);
			return $rs;
    ...
    pg_free_result($result);
	 ...
	 function projects(){
		...
		$dept_name = $_POST['dept'];
		if ($dept_name == 'all'){
			$query = "select * from sp_index_v2 order by name_of_the_project asc";
		}else {
			$query = "select * from sp_index_v2 where hod_name = '$dept_name' order by name_of_the_project asc";
		}
		$result = pg_query($dbacon, $query);
		$count = pg_num_rows($result);
